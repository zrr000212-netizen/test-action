...
const PRIMARY_ENV_PATH = path.join(os.homedir(), '.config', 'agent-mail-kit', '.env');
...
function buildConfig(env, prefix) {
  ...
  if (prefix && !env[`${p}IMAP_HOST`]) {
    console.error(`Error: Account "${prefix.toLowerCase()}" not found in config. Check ~/.config/agent-mail-kit/.env`);
    process.exit(1);
  }
